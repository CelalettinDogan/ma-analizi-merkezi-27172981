...
const DonutChart: React.FC<{
  ...
  const homeLength = (homeWins / total) * circumference;
  const drawLength = (draws / total) * circumference;
  const awayLength = (awayWins / total) * circumference;
  ...
  return (
    <div className="relative flex items-center justify-center">
      <svg width="100" height="100" className="transform -rotate-90">
        <circle cx="50" cy="50" r={radius} fill="none" stroke="hsl(var(--muted))" strokeWidth="12" opacity="0.3" />
        <motion.circle cx="50" cy="50" r={radius} fill="none" stroke="hsl(var(--primary))" strokeWidth="12"
          strokeDasharray={`${homeLength} ${circumference}`} strokeDashoffset={0} strokeLinecap="round"
          initial={{ strokeDasharray: `0 ${circumference}` }} animate={{ strokeDasharray: `${homeLength} ${circumference}` }}
          transition={{ duration: 1, delay: 0.3, ease: "easeOut" }} />
        <motion.circle cx="50" cy="50" r={radius} fill="none" stroke="hsl(var(--muted-foreground))" strokeWidth="12"
          strokeDasharray={`${drawLength} ${circumference}`} strokeDashoffset={drawOffset} strokeLinecap="round"
          initial={{ strokeDasharray: `0 ${circumference}` }} animate={{ strokeDasharray: `${drawLength} ${circumference}` }}
          transition={{ duration: 1, delay: 0.5, ease: "easeOut" }} />
        <motion.circle cx="50" cy="50" r={radius} fill="none" stroke="hsl(var(--secondary))" strokeWidth="12"
          strokeDasharray={`${awayLength} ${circumference}`} strokeDashoffset={awayOffset} strokeLinecap="round"
          initial={{ strokeDasharray: `0 ${circumference}` }} animate={{ strokeDasharray: `${awayLength} ${circumference}` }}
          transition={{ duration: 1, delay: 0.7, ease: "easeOut" }} />
      </svg>
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <motion.span className="text-lg font-bold text-foreground"
    ...
    <div className="flex flex-col gap-2 flex-1 min-w-0">
      {stats.map((stat, index) => (
        <motion.div key={stat.label} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
          transition={{ delay: 0.4 + index * 0.1 }} className="flex items-center gap-2">
          <div className={cn("w-2.5 h-2.5 rounded-full shrink-0", stat.color)} />
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-1">
              <span className="text-xs text-muted-foreground truncate max-w-[80px]">{stat.label}</span>
              <div className="flex items-center gap-1.5 shrink-0">
                <span className={cn("text-sm font-bold", stat.textColor)}>{stat.value}</span>
                <span className="text-micro text-muted-foreground/70">({stat.percentage}%)</span>
              </div>
            </div>
            <div className="h-1 bg-muted/50 rounded-full mt-1 overflow-hidden">
              <motion.div className={cn("h-full rounded-full", stat.color)}
...
const H2HTimeline: React.FC<H2HTimelineProps> = ({ h2h, homeTeam, awayTeam }) => {
  ...
  const getStreak = () => {
    ...
    if (streak >= 2 && streakType) return { count: streak, type: streakType };
    return null;
  };
          ...
          <div className="overflow-x-auto scrollbar-none -mx-1 px-1">
            <div className="flex justify-between gap-3 min-w-0">
              {lastMatches.slice(0, 5).map((match, index) => (
                <MatchBubble key={index} match={match} result={getMatchResult(match)} index={index} />
              ))}
            </div>
          </div>
        </div>
