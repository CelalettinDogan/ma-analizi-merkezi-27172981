...
const AIPreviewBadge: React.FC<{ match: Match }> = ({ match }) => {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-primary/6 border border-primary/8 mt-3">
      <Sparkles className="w-3.5 h-3.5 text-primary shrink-0" />
      <span className="text-micro font-semibold text-primary/70">AI Tahmin</span>
  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        ...
        <div className="h-44 rounded-2xl bg-muted/15 animate-pulse" />
        <div className="space-y-2">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="h-16 rounded-2xl bg-muted/10 animate-pulse" style={{ animationDelay: `${i * 100}ms` }} />
          ))}
        </div>
      </div>
    );
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground/60 bg-muted/30 px-2 py-0.5 rounded-full">{matches.length}</span>
        </div>
      </div>

      {/* Featured Match Card — Premium surface */}
      {featuredMatch && (
        <motion.button
        ...
        >
          ...
          <div className="flex items-center justify-between flex-wrap gap-1.5 mb-5">
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-micro font-medium bg-muted/40 text-foreground/70">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 flex items-center gap-3 min-w-0">
              <div className="w-12 h-12 rounded-2xl bg-muted/20 flex items-center justify-center overflow-hidden flex-shrink-0">
              ...
              <span className="font-semibold text-sm leading-tight line-clamp-2 min-w-0">
            ...
            <div className="flex flex-col items-center flex-shrink-0 px-2">
              ...
              <span className="text-micro text-muted-foreground/50 mt-0.5">
            ...
            <div className="flex-1 flex items-center gap-3 min-w-0 justify-end">
              <span className="font-semibold text-sm leading-tight line-clamp-2 min-w-0 text-right">
          ...
          {/* CTA row — stronger button */}
          <div className="flex items-center justify-center mt-4 pt-4 border-t border-border/15">
            <span className="inline-flex items-center gap-2 bg-primary text-primary-foreground rounded-xl px-5 py-2.5 shadow-sm shadow-primary/15">
      ...
      {/* Match List — card rows with spacing */}
        ...
        {displayedMatches.map((match, index) => {
          ...
          return (
            <React.Fragment key={match.id}>
              {showDateSeparator && (
                <div className="text-micro text-muted-foreground/60 font-medium px-3 py-2 mt-3">
              ...
              <motion.button
                ...
                className={cn(
                  "w-full flex items-center gap-3 px-3.5 py-3.5 rounded-2xl transition-colors text-left",
                  "min-h-[60px] bg-card/50 border border-border/20",
                  isAnyLoading && !isThisLoading && "opacity-40",
                  isThisLoading && "bg-primary/5 border-primary/15",
                  !isAnyLoading && "active:bg-muted/30"
                )}
              >
                {/* Time */}
                <span className={cn(
                  "text-xs font-semibold w-11 shrink-0 tabular-nums",
                  isThisLoading ? "text-primary" : "text-muted-foreground/60"
                )}>
                  {matchTime}
                </span>
                ...
                <div className="flex-1 min-w-0 flex items-center gap-1.5">
                  ...
                  <span className="text-muted-foreground/30 text-xs shrink-0 px-0.5">–</span>
                  {/* Away */}
                  <div className="flex items-center gap-1.5 min-w-0 flex-1">
                    <span className="text-sm truncate min-w-0 font-medium">
                ...
                <span className="text-micro text-muted-foreground/40 shrink-0 font-medium">
      ...
      {otherMatches.length > 5 && !showAll && (
        <Button
          ...
          className="w-full text-primary text-xs h-10 rounded-xl"
        >
