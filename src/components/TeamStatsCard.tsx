...
const FormIcon: React.FC<{ result: string }> = ({ result }) => {
  ...
  switch (result) {
    case 'W':
      return <span className="w-6 h-6 rounded-full bg-win/20 text-win flex items-center justify-center text-xs font-bold">{t('stats.formLetters.W')}</span>;
    case 'L':
      return <span className="w-6 h-6 rounded-full bg-loss/20 text-loss flex items-center justify-center text-xs font-bold">{t('stats.formLetters.L')}</span>;
    case 'D':
      return <span className="w-6 h-6 rounded-full bg-draw/20 text-draw flex items-center justify-center text-xs font-bold">{t('stats.formLetters.D')}</span>;
    default:
      return null;
  }
};
    ...
    <div className="glass-card p-6 animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        ...
        <span className={`prediction-badge ${isHome ? 'bg-primary/20 text-primary' : 'bg-secondary/20 text-secondary'}`}>
          {isHome ? t('teams.home') : t('teams.away')}
        </span>
      </div>

      {/* Son 5 Maç Formu */}
      <div className="mb-6">
        <p className="text-sm text-muted-foreground mb-3">{t('stats.last5')}</p>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="stat-card">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-muted-foreground">{t('stats.goalsScoredAvg')}</span>
          </div>
        ...
        <div className="stat-card">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-muted-foreground">{t('stats.goalsConcededAvg')}</span>
          </div>
